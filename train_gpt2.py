...
class CasualSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        self.c_attn = nn.Linear(config.n_embd, config.n_embd * 3) # key, query, value
        self.c_proj = nn.Linear(config.n_embd, config.n_embd) # ouput projection

        # Regularization
        self.n_head = config.n_head
        self.n_enmbd = config.n_embd
    ...
    def forward(self, x):
        ...
        qkv = self.c_attn(x)
        q, k, v = qkv.split(self.n_enmbd, dim=2)
